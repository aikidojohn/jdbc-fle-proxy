...
public class FF1Test {
    ...
    @Test
    public void testWithRadix95() {
        byte[] keyBytes = DatatypeConverter.parseHexBinary("2B7E151628AED2A6ABF7158809CF4F3C");
        SecretKey key = new SecretKeySpec(keyBytes, "AES");
        String encrypted = FF1Encryption.encrypt(key, new byte[]{55, 55, 55, 55, 112, 113, 114, 115, 55, 55, 55}, "The quick brown fox jumps over the lazy dog!", Alpha.ASCII_PRINTABLE );
        Assert.assertEquals("*o&]10/uu~e:|W''B3x?;syZ|JWkfRuxKD~w;laGv0@q", encrypted);

        String plaintext = FF1Encryption.decrypt(key, new byte[]{55, 55, 55, 55, 112, 113, 114, 115, 55, 55, 55}, "*o&]10/uu~e:|W''B3x?;syZ|JWkfRuxKD~w;laGv0@q", Alpha.ASCII_PRINTABLE );
        Assert.assertEquals("The quick brown fox jumps over the lazy dog!", plaintext);
    }


    @Ignore
    @Test
    public void testWithRadix95Max() throws Exception {
        byte[] keyBytes = DatatypeConverter.parseHexBinary("2B7E151628AED2A6ABF7158809CF4F3C");
        SecretKey key = new SecretKeySpec(keyBytes, "AES");
        String longText = Resources.toString(Resources.getResource("LongText.txt"), Charset.forName("UTF-8"));
        String longCiphertext = Resources.toString(Resources.getResource("LongCiphertext.txt"), Charset.forName("UTF-8"));

        long start = System.currentTimeMillis();
        String encrypted = FF1Encryption.encrypt(key, new byte[]{55, 55, 55, 55, 112, 113, 114, 115, 55, 55, 55}, longText, Alpha.ASCII_PRINTABLE );
        long end = System.currentTimeMillis();
        System.out.println("Time: " + (end - start));
        Assert.assertEquals(longCiphertext, encrypted);

        start = System.currentTimeMillis();
        String plaintext = FF1Encryption.decrypt(key, new byte[]{55, 55, 55, 55, 112, 113, 114, 115, 55, 55, 55}, longCiphertext, Alpha.ASCII_PRINTABLE );
        end = System.currentTimeMillis();
        System.out.println("Time: " + (end - start));
        Assert.assertEquals(longText, plaintext);
    }
}
